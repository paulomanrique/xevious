...
__vblank:
        jmp     md_vblank               /* the Xevious OSD vblank handler */
...
* === Xevious 32X boot shim ===
* Enable SH2 access to the MARS hardware and let the SH2s run, then hand off to
* the Mega Drive init (startup.68k _start): it sets the Xevious VDP layout,
* clears VRAM/CRAM/VSRAM, sets the stack, and jumps to md_main (the game).
        ...
        move.w  0xA15100,d0
        or.w    #0x8000,d0
        move.w  d0,0xA15100             /* set FM - SH2 access to MARS hw */
        move.l  #0,0xA15120             /* let Master SH2 run */

        jmp     _start                  /* -> startup.68k: MD init + jmp md_main */
