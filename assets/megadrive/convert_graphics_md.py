...
def main():
    ...
    frames = []
    frames32x = []          # parallel to frames (same fid): 8bpp full arcade colour
    frame_ids = {}
    sp_entries = {}
    for idx, clut, baked in sp_combos:
        ...
        if fid is None:
            ...
            # 32X target: the same frame in 8bpp with the EXACT arcade colours
            # (pixel = arcade palette idx + 1; 0 = transparent), canonicalised to
            # match `can` so it shares the MD frame id (the 68000 resolves once).
            cl = sprite_cluts[clut]
            arc = [0 if cl[p] >= 128 else cl[p] + 1 for p in sprites[idx]]
            if hf:
                arc = list(flip_h(arc, 16, 16))
            if vf:
                arc = list(flip_v(arc, 16, 16))
            frames32x.append(bytes(arc))
        sp_entries[(idx, clut)] = (fid, pal, hf, vf)
    report.append(f"sprites: {len(sp_combos)} combos -> {len(frames)} frames "
                  f"({len(frames)*128} bytes)")

    # ---------- 32X sprite assets (8bpp + full arcade palette, by MD frame id) ----------
    ...
    if os.path.isdir(os.path.dirname(sega_dir)):
        ...
        pal32x = bytearray()
        for r, g, b in palette:                       # arcade 8-bit RGB -> 32X 5:5:5
            w = ((r >> 3) & 0x1F) | (((g >> 3) & 0x1F) << 5) | (((b >> 3) & 0x1F) << 10)
            pal32x += bytes((w >> 8, w & 0xFF))       # big-endian for the SH-2
        with open(os.path.join(sega_dir, "gfx_32x_palette.bin"), "wb") as f:
            f.write(bytes(pal32x))
        with open(os.path.join(sega_dir, "gfx_32x_sprites.bin"), "wb") as f:
            f.write(b"".join(frames32x))
        with open(os.path.join(sega_dir, "gfx_32x_data.s"), "w") as f:
            f.write("! generated by assets/megadrive/convert_graphics_md.py - DO NOT EDIT\n")
            f.write(f"! {len(palette)} arcade colours, {len(frames32x)} sprite frames"
                    " (8bpp, numbered by MD frame id)\n\n")
            f.write("    .global _gfx_32x_palette\n")
            f.write("    .global _gfx_32x_sprites\n")
            f.write("    .global _gfx_32x_nframes\n\n")
            f.write("    .section .rodata\n\n")
            f.write("    .align 2\n_gfx_32x_palette:\n    .incbin \"src/gfx_32x_palette.bin\"\n\n")
            f.write("    .align 2\n_gfx_32x_sprites:\n    .incbin \"src/gfx_32x_sprites.bin\"\n\n")
            f.write("    .align 2\n_gfx_32x_nframes:\n    .long " + str(len(frames32x)) + "\n")
        report.append(f"32X: {len(frames32x)} frames ({len(frames32x)*256} bytes), "
                      f"{len(palette)}-colour palette")
    ...
    def build_egg():
        ...
        # ---- 32X full-colour egg: the whole 320x224 screen, up to 255 colours
        # (vs the MD's ~28). Pixel value = palette idx + 1 (1..255; 0 stays the
        # transparent slot). The SH-2 blits this image straight to its framebuffer.
        sega_dir = os.path.normpath(os.path.join(this_dir, "..", "..", "src", "sega32x", "src"))
        if os.path.isdir(os.path.dirname(sega_dir)):
            os.makedirs(sega_dir, exist_ok=True)
            q = canvas.quantize(colors=255, dither=0)
            with open(os.path.join(sega_dir, "gfx_egg_32x.bin"), "wb") as f:
                f.write(bytes((i + 1) for i in q.getdata()))
            rp = q.getpalette() or []
            palb = bytearray()
            for i in range(255):
                r0, g0, b0 = rp[i * 3:i * 3 + 3] if i * 3 + 2 < len(rp) else (0, 0, 0)
                w = ((r0 >> 3) & 0x1F) | (((g0 >> 3) & 0x1F) << 5) | (((b0 >> 3) & 0x1F) << 10)
                palb += bytes((w >> 8, w & 0xFF))
            with open(os.path.join(sega_dir, "gfx_egg_32x_pal.bin"), "wb") as f:
                f.write(bytes(palb))
            with open(os.path.join(sega_dir, "gfx_egg_32x.s"), "w") as f:
                f.write("! generated by assets/megadrive/convert_graphics_md.py - DO NOT EDIT\n\n")
                f.write("    .global _gfx_egg_32x\n    .global _gfx_egg_32x_pal\n\n")
                f.write("    .section .rodata\n\n")
                f.write("    .balign 4\n_gfx_egg_32x:\n    .incbin \"src/gfx_egg_32x.bin\"\n\n")
                f.write("    .align 2\n_gfx_egg_32x_pal:\n    .incbin \"src/gfx_egg_32x_pal.bin\"\n")
            report.append("32X egg: 320x224 image, 255-colour palette")
